...
| `VAD_TRAILING_SILENCE_S` | How long the array must report no voice before a recording ends (default `0.8`). Raise it if people get cut off mid-sentence; every turn then waits that much longer before the answer starts |
...
| `pose_states.py` | Holds the assistant in each visible state (idle, listening, speaking, redirected, room clear) one at a time so each can be photographed; LED commands and timings come from the production handlers, so the poses match the deployed behaviour |
...
| `analyse_head_accuracy.py` | Analyses the Study 1 CSVs: error decomposition, Kruskal-Wallis, optional plots; `--trials` rebuilds each head move from the trial records to compare estimators and split latency (`--moves` lists them per trial, `--pgf-until` prints plot coordinates, `--only`/`--drop` select cells or leave trials out) |
| `study_privacy_switch.py` | Runs Study 2 (privacy switch and door-zone ducking) with the live radar and TTS; `--report` analyses a session, `--list`/`--drop` review or remove trials, `--simulate` rehearses without hardware. `--recording` plays `study/private_reply.wav` instead of live TTS; create that file once with `--render-recording`, which speaks the message through the live TTS provider and active voice and notes voice, model and file hash in `study/private_reply.json`. Every trial record carries the reply source, the exit rule and the git commit; redone trials are logged with their reason to `privacy_switch_<session>_discarded.jsonl` |
| `analyse_privacy_switch.py` | Analyses a Study 2 session from its CSV and trial log: switch rates with exact CIs, entry timing, premature vs. missed returns to speech (the harness's `--report` only sees returns after the leave cue), door-zone depth of peeks and passes; `--coords` / `--track ID` print the figure data. No third-party dependencies |
| `replay_privacy_switch.py` | Replays a Study 2 session's recorded radar frames through the current entry/exit and privacy logic and compares every trial with what happened live. `--radar-module` replays an older `radar_ld2450.py` instead (e.g. `git show <rev>:adapters/hardware/radar_ld2450.py > old.py`) — the way to check a change to the tracking logic against real recordings without a new session |
| `replay_headcount.py` | Replays a Study 2 session through a head-count policy instead of the door-zone logic and reports what it would have done, against what ran live |
| `render_radar_frames.py` | Renders chosen moments of a Study 2 trial as TikZ panels (room and door zones, targets, track so far), to pair photographs of an entry with what the radar saw |
...
Servo motion is deliberately not 1:1 with raw DOA readings — `--doa` only reacts while the mic's onboard VAD (`RespeakerDOAAdapter.get_voice_active()`) says something voice-like is happening. On top of that, `service_layer/handlers.py`'s `start_doa_tracking()` takes a median of several DOA samples (`doa_samples`, default 5) — 250 ms apart, after discarding the first 1.5 s of voice so the array's estimate has settled — rather than reacting to a single reading. That median is re-expressed as the shortest turn towards the talker, so a voice from beside or behind the head turns it as far that way as the sweep allows instead of to the opposite end stop, and `ServoTurntableAdapter._ramp_to()` glides to the target over small intermediate PWM steps (`steps_per_s`, default 20) instead of jumping in one update. Tune those if it feels too sluggish or still too twitchy — but if the servo is buzzing/jittering while *holding* a constant angle (no new movement in the log either), that's PWM signal jitter, not these constants. Set `SERVO_USE_PIGPIO=true` and install/enable the daemon so it survives a reboot:
