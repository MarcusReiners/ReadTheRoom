...
- **Production now uses Study 1's settle-first settings (changed 15 September 2026).** `start_doa_tracking` defaults: `doa_onset_skip_s=1.5`, `doa_sample_interval_s=0.25`, `min_doa_samples=3`, `post_move_quiet_s=1.5`, `sample_window_s=6.0`, `doa_samples=5`, and `main.py` passes the servo's safe range as `accept_range`. These match Study 1's main and validation sessions. The earlier defaults (`doa_onset_skip_s=0.0`, `sample_window_s=0.6`, `min_doa_samples=2`, `post_move_quiet_s=0.6`, no range check) are what the 14 September production session and the 60° calibration-table runs measured. The main session ran raw bearings (no off-axis gain); the validation session ran `offaxis_gain=0.869`.
